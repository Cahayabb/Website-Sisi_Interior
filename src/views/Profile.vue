...
const fetchProfile = async () => {
  try {
    const res  = await fetch(`${BASE_URL}/users/profile`, { headers: getHeaders() })
...
const handleSimpan = async () => {
  ...
  try {
    ...
    localStorage.setItem('user', JSON.stringify({
      ...
      email:      form.email,
      username:   data.user?.username || JSON.parse(localStorage.getItem('user') || '{}').username || '',
      no_telepon: form.noTelepon,
      foto_url:   data.user?.foto_url || avatarPreview.value || '',
      role:       'users',
    }))
