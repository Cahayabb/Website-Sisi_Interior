      ...
      <div class="table-card">
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-for="(item, index) in paginatedList" :key="item.data.ID">
              <td class="td-no">{{ (currentPage - 1) * perPage + index + 1 }}</td>
              <td class="td-main">{{ item.data.jenis_ruangan}}</td>
              <td class="td-cell">
                <span class="badge" :class="badgeClass(item.data.jenis_proyek)">{{ item.data.luas_area }} m</span>
              </td>
              <td class="td-cell">{{ formatRupiah(item.data.harga_proyek || 0) }}</td>
              <td class="td-cell td-file">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                {{ item.data.spesifikasi_design}}
              </td>
              <td class="td-cell td-muted">{{ formatTanggal(item.data.created_at) }}</td>
              <td class="td-aksi">
                ...
                <button class="action-btn action-btn--delete" @click="deleteItem(item.data.ID)" title="Hapus">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
      ...
      <div class="form-layout">
        ...
        <div class="form-card">
          <div class="form-grid-2">
            <div class="field">
              <label class="field__label">Nama Perusahaan <span class="required-asterisk">*</span></label>
          ...
          <div class="field">
            <label class="field__label">Tingkat Kerumitan <span class="required-asterisk">*</span></label>
            <div class="field__select-wrap">
          ...
          <div class="field">
            <label class="field__label">Durasi Pengerjaan (hari) <span class="required-asterisk">*</span></label>
            <input v-model="form.durasi_pengerjaan" type="number" class="field__input" placeholder="" />
          </div>

          <div class="field">
            <label class="field__label">Lokasi Proyek <span class="required-asterisk">*</span></label>
...
const fetchEstimasiList = async () => {
  ...
  try {
    ...
    console.log(estimasiList.value);
...
const payload = {
      ...
      jenis_ruangan: (form.value.jenis_ruangan || []).join(", "),
    ...
    const estimasi = Number(data.rekomendasi ?? data.estimasi ?? 0)
    const minimum = Number(data.minimum ?? (estimasi * 0.8))
    const maksimum = Number(data.maksimum ?? (estimasi * 1.2))
...
const viewHasil = (item) => {
  ...
  activeItem.value = item.data

  // Isi hasilEstimasi dari harga_proyek tersimpan
  const harga = item.data.harga_proyek || 0
  hasilEstimasi.value = {
    minimum: harga * 0.8,
    maksimum: harga * 1.2,
    rekomendasi: harga
...
// })
  ...
  if (filterJenis.value) {
    list = list.filter(p =>
      p.data?.jenis_proyek === filterJenis.value  // ← tambah .data
    )
  }
  ...
  if (sortBy.value === 'nama_asc') {
    list.sort((a, b) =>
      (a.data?.nama_perusahaan || '').localeCompare(b.data?.nama_perusahaan || '')
    )
  }
...
.form-grid-2 .field:nth-child(2) .field__label::after {
  content: " *";
  color: #D64545;
  font-size: 11px;
  vertical-align: super;
